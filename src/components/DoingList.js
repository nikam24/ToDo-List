...
export default function DoingList({ tasks, setTasks }) {
...
const onDelete = (id) => {
  ...
  return (
    <div
      className="doing-container self-start min-h-[100px] relative bg-gray-100 shadow-2xl shadow-slate-700 border-black rounded-sm w-72"
      onDragOver={handleDragOver}
      onDrop={handleDrop}
    >
      <h2 className='absolute m-2 text-blue-900 font-semibold'>Doing</h2> <br />
      {taskList.map((task) => (
        <div
          key={task.id}
          draggable
      ...
      {!showInputField && <button onClick={handleAddTaskClick} className=' text-blue-900 font-semibold relative bottom-0 m-2 mb-2 right-20'>+ Add a Card</button>}
    </div>
  );
}
